...
func (a *App) apiImportHandler(w http.ResponseWriter, r *http.Request) {
	...
	var body struct {
		...
		// Fuzzy is accepted for wire compatibility with older clients but no
		// longer changes import behavior: it previously routed through a set
		// of wrapper functions that called the exact same import path with
		// no actual fuzzy-matching logic, while also rejecting several
...
// The XLSX writer/reader codec (writeXLSX, xlsxToCSV, and their helpers)
// lives in xlsx.go.
